...
- (LLDBProcess *)instantiateProcessByAttachingToProcessWithID:(uint64_t)pid error:(LLDBError *__autoreleasing *)error
{
...
@implementation LLDBTarget (LLDBObjectiveCExtensions)
- (NSArray *)allModules
{
	NSArray*	a1	=	@[];
	NSUInteger	c1	=	[self numberOfModules];
	for (NSUInteger i=0; i<c1; i++)
	{
		a1	=	[a1 arrayByAddingObject:[self moduleAtIndex:i]];
	}
	return	a1;
}
...
{
	NSArray*	a1	=	@[];
	NSUInteger	c1	=	[self numberOfBreakpoints];
	for (NSUInteger i=0; i<c1; i++)
	{
		a1	=	[a1 arrayByAddingObject:[self breakpointAtIndex:i]];
	}
	return	a1;
}
@end
